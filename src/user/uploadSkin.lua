...
return function (request)
    ...
    local file = request.req.params.file
    if not file then
        return errors.UnsupportedMediaType{cause="Missing file"}
    end
    local variant = request.req.params.variant
    if not variant then
        return errors.UnsupportedMediaType{cause="Missing variant"}
    end
    ...
    local filepath = "data/skins"
    if not lfs.exists(filepath) then
        lfs.mkdir(filepath)
    end

    filepath = filepath+"/"+luasodium.crypto_generichash(file)+".png"
